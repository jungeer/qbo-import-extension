...
- 仅接受 `.csv`；文件名包含 `USD` 时，导入前先将页面币种下拉切换为 USD
- 支持选择文件夹，自动列出文件夹内全部 CSV 并按文件名排序导入
- 面板顶部 Tab 切换「导入 / 配置」；配置页可自定义选择器、USD 规则、等待时间并持久化
...
5. **在 QBO / 业务页使用**：先打开目标网页，再点浏览器工具栏里的扩展图标（不要在 `chrome://`、扩展管理页、Chrome 网上应用店页面点）
...
| 原因 | 说明 |
|------|------|
| **Demo 页特殊** | `demo/index.html` 自带脚本按钮，不依赖扩展注入，所以 demo 总能用 |
| **未在目标页点图标** | 必须在你要操作的 QBO 标签页里点击扩展图标 |
| **受限页面** | `chrome://`、`edge://`、应用商店、部分 PDF/内置页无法注入 |
| **未刷新扩展** | 改代码后需在 `chrome://extensions` 点「重新加载」 |
| **本地 file://** | 需在扩展详情里开启「允许访问文件网址」 |

旧版曾把样式资源限制在 `*.intuit.com`，**无法匹配** `app.qbo.intuit.com` 这类多级子域名；现已改为 `insertCSS` + `<all_urls>`。
